...
import PlanDetailClient from "./PlanDetailClient"; // adjust import path
...
import { getParticipantCapacity, getJoinedParticipantsCount } from "@/lib/plan";
...
export async function generateMetadata({
  ...
  // ✅ plan.participants is fetched above — this will be correct
  ...
  const ogImage =
    plan.image_url ||
    `${SITE_URL}/api/og-standard?title=${encodeURIComponent(
      plan.title,
    )}&city=${encodeURIComponent(plan.city || "")}&date=${encodeURIComponent(
...
export default async function Page({
  ...
  // ✅ typed as Promise and awaited — consistent with generateMetadata
  const { shortId } = await params;
  const cookieStore = await cookies();
  const supabase = createClient(cookieStore);
  const { data: auth } = await supabase.auth.getUser();

  // ✅ First resolve short_id → full plan with host
  ...
  // ✅ Now use plan.id (UUID) for all relational queries — same as original
