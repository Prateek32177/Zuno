...
export async function PATCH(
  ...
  if (!existing)
    return NextResponse.json({ error: "Plan not found" }, { status: 404 });
  if (existing.host_id !== auth.user.id)
    return NextResponse.json(
      { error: "Only host can edit this plan" },
      { status: 403 },
    );
  ...
  const mapPlanStatus = (
    status: unknown,
  ): "active" | "full" | "completed" | "cancelled" | undefined => {
  ...
  const allowed: Record<string, any> = {
    ...
    whatsapp_link:
      typeof body.whatsapp_link === "string"
        ? body.whatsapp_link.trim()
        : body.whatsapp_link,
    ...
    cost_amount:
      body.cost_amount !== undefined ? Number(body.cost_amount) : undefined,
    final_amount:
      body.final_amount !== undefined ? Number(body.final_amount) : undefined,
    host_included_in_spots_and_splits:
      body.host_included_in_spots_and_splits === undefined
        ? undefined
        : !!body.host_included_in_spots_and_splits,
...
export async function DELETE(
  ...
  if (existing.host_id !== auth.user.id)
    return NextResponse.json(
      { error: "Only host can delete this plan" },
      { status: 403 },
    );
